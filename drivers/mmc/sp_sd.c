...
static int Sd_drv_Bus_Reset_Channel(struct sp_mmc_host *host);
static int Reset_drv_Controller(struct sp_mmc_host *host);
static void sp_drv_mmc_prep_cmd_rsp(struct sp_mmc_host *host, struct mmc_cmd *cmd);
static void sp_drv_mmc_prep_data_info(struct sp_mmc_host *host, struct mmc_cmd *cmd, struct mmc_data *data);
static void sp_drv_mmc_wait_sdstate_new(struct sp_mmc_host *host);
...
static void sp_mmc_dcache_flush_invalidate(struct mmc_data *data, ulong phys_start, unsigned int data_len)
{
...
void send_drv_stop_cmd(struct sp_mmc_host *host)
{
	struct mmc_cmd stop = {};
	...
	sp_drv_mmc_prep_cmd_rsp(host, &stop);
...
sp_mmc_send_cmd(struct mmc *mmc, struct mmc_cmd *cmd, struct mmc_data *data)
{
	...
	for (i = 0; i < CMD_MAX_RETRY; ++i)
	{
		...
		if (data == NULL) {
			...
			sp_sd_trace();
			sp_drv_mmc_prep_data_info(host, cmd, data);
			sp_mmc_trigger_sdstate(host);
			...
			if(SP_MMC_PIO_MODE == host->dmapio_mode) {
				sp_drv_mmc_read_data_pio(host, data);
			}
...
#ifdef CONFIG_DM_MMC
const struct dm_mmc_ops sp_drv_mmc_ops = {
...
static int sp_drv_sd_hw_init(sp_mmc_host *host)
{
...
static int sp_drv_sd_hw_set_clock(struct sp_mmc_host *host, uint div)
{
...
static int sp_drv_sd_hw_tunel_read_dly (sp_mmc_host *host, sp_mmc_timing_info *dly)
{
...
static int sp_drv_sd_hw_tunel_write_dly  (sp_mmc_host *host, sp_mmc_timing_info *dly)
{
...
static int sp_drv_sd_hw_tunel_clock_dly  (sp_mmc_host *host, sp_mmc_timing_info *dly)
{
	host->base->sd_clk_dly_sel = dly->clk_dly;
	return 0;
}

int sp_drv_sd_hw_highspeed_en (sp_mmc_host *host, bool en)
{
	if (en)
...
int sp_drv_sd_hw_set_bus_width (sp_mmc_host *host, uint bus_width)
{
	/* Set the bus width */
	if (bus_width == 4) {
...
int sp_drv_sd_hw_set_sdddr_mode (sp_mmc_host *host, int ddrmode)
{
	sp_sd_trace();
	if (ddrmode)
...
int sp_drv_sd_hw_set_cmd (sp_mmc_host *host, struct mmc_cmd *cmd)
{
	...
	host->base->sd_cmdbuf2 = (u8)((cmd->cmdarg >> 16) & 0xff);
...
int sp_drv_sd_hw_get_reseponse (sp_mmc_host *host, struct mmc_cmd *cmd)
{
	...
	if (cmd->resp_type & MMC_RSP_136) {
		...
		rspBuf[0] = host->base->sd_rspbuf0;
		rspBuf[0] = host->base->sd_rspbuf1;
		rspBuf[1] = host->base->sd_rspbuf2;
		rspBuf[2] = host->base->sd_rspbuf3;
		rspBuf[3] = host->base->sd_rspbuf4;
		rspBuf[4] = host->base->sd_rspbuf5;
...
int sp_drv_sd_hw_set_data_info (sp_mmc_host *host, struct mmc_cmd *cmd, struct mmc_data *data)
{
	ulong hw_address;
	/* Reset */
	Reset_drv_Controller(host);
...
#define SP_MMC_DMA_MASK			((0x3ul<< SP_MMC_DMADST_SHIFT) | (0x03ul << SP_MMC_DMASRC_SHIFT))
		...
		host->base->medatype_dma_src_dst &= ~SP_MMC_DMA_MASK;
	/* Configure Group DMA Registers */
	if (data->flags & MMC_DATA_WRITE) {
			host->base->medatype_dma_src_dst |= SP_MMC_DMA_TO_DEVICE;
#if 0
		...
		hw_address = (ulong) data->src;
			...
			host->base->medatype_dma_src_dst |= SP_MMC_DMA_FROM_DEVICE;
#if 0
		...
		host->base->dma_base_addr = hw_address;
		SP_MMC_SECTOR_NUM_SET(host->base->sdram_sector_0_size, data->blocks);
	}
...
int sp_drv_sd_hw_trigger (sp_mmc_host *host)
{
	host->base->sdctrl0 = 1;   /* Start transaction */
	return 0;
}

int sp_drv_sd_hw_reset_mmc (sp_mmc_host *host)
{
	sp_sd_trace();
...
int sp_drv_sd_hw_reset_dma (sp_mmc_host *host)
{
...
int sp_drv_sd_hw_reset_sdio (sp_mmc_host *host)
{
	return 0;
}

int sp_drv_sd_hw_wait_data_timeout (sp_mmc_host *host, uint timeout)
{
	return 0;
}

int sp_drv_sd_hw_check_finish (sp_mmc_host *host)
{
...
int	sp_drv_sd_hw_tx_dummy (sp_mmc_host *host)
{
	host->base->sdctrl1 = 1;
	return 0;
}

int sp_drv_sd_hw_check_error (sp_mmc_host *host, bool with_data)
{
	int ret = 0;
...
sp_mmc_hw_ops sd_drv_hw_ops = {
	...
	.highspeed_en		= sp_drv_sd_hw_highspeed_en,
	.tunel_read_dly		= sp_drv_sd_hw_tunel_read_dly,
	.tunel_write_dly	= sp_drv_sd_hw_tunel_write_dly,
	.tunel_clock_dly	= sp_drv_sd_hw_tunel_clock_dly,
	.set_bus_width		= sp_drv_sd_hw_set_bus_width,
	.set_sdddr_mode		= sp_drv_sd_hw_set_sdddr_mode,
	.set_cmd			= sp_drv_sd_hw_set_cmd,
	.get_response		= sp_drv_sd_hw_get_reseponse,
	.set_data_info		= sp_drv_sd_hw_set_data_info,
	.trigger			= sp_drv_sd_hw_trigger,
	.reset_mmc			= sp_drv_sd_hw_reset_mmc,
	.reset_dma			= sp_drv_sd_hw_reset_dma,
	.reset_sdio			= sp_drv_sd_hw_reset_sdio,
	.wait_data_timeout	= sp_drv_sd_hw_wait_data_timeout,
	.check_finish		= sp_drv_sd_hw_check_finish,
	.tx_dummy			= sp_drv_sd_hw_tx_dummy,
...
static int sp_mmc_probe(struct udevice *dev)
{
	...
	ret = clk_enable(&clk);
	if (ret < 0) {
		IFPRINTK("failed to enable sunplus SD clk\n");
		return ret;
	}
...
int sp_drv_print_mmcinfo(struct mmc *mmc)
{
	struct sp_mmc_host *host = mmc->priv;
...
int sp_drv_mmc_set_dmapio(struct mmc *mmc, uint val)
{
	struct sp_mmc_host *host = mmc->priv;
