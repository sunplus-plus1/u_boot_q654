...
struct sp_pnand_info {
	struct nand_chip nand;
	void __iomem *io_base;
	void __iomem *sram_base;
	int sel_chip;//chip enable 0-7
	int cur_cmd;
	int page_addr;
	int column;
	int byte_ofs;
	...
	int large_page;
	int eccbasft;
	int max_spare; //max spare data bytes (PNANDC v2.4: 32/128 bytes)
	int spare_ch_offset; //register 0x1000 channel offset (PNANDC v2.4: 0x80 = shift 7)
	int spare;
	int protect_spare;
	int useecc;
	int useecc_spare;
	...
	int (*write_page) (struct nand_chip *nand, const uint8_t *buf);
	int (*read_page) (struct nand_chip *nand, uint8_t *buf);
	int (*set_param) (struct nand_chip *nand);
	int (*terminate) (struct nand_chip *nand);
	unsigned long priv;
};
...
extern int sp_pnand_read_page(struct mtd_info *, struct nand_chip *,
			      uint8_t *, int, int);
extern int sp_pnand_write_page_lowlevel(struct mtd_info *,
					struct nand_chip *, const uint8_t *, int, int);
extern int sp_pnand_read_oob_std(struct mtd_info *,
